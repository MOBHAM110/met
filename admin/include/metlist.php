...
foreach($sidebarcolumn as $key=>$val){
	if($val[name]=='lang_memberManage')continue;
	if($val[name]=='lang_memberset' || $val[name] == 'lang_memberarrayManage')continue;
	if($val[name] == 'lang_indexebook'||$val[name] == 'lang_indexbbs'||$val[name] == 'lang_indexcode')continue;
	if($val[name] == 'lang_temstyle'||$val[name]=='lang_indexskinset'||$val[name] == 'lang_indexwebcount')continue;
	if($val[name] == 'lang_indexhtmset')continue;
	...
	if((($val[name]=='lang_webnanny')||($val[name]=='lang_smsfuc'))&&$met_agents_type>1)continue;
