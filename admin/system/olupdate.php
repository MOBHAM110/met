...
if($action=="check"){
	...
	if($return!=1){		
		dl_error($lang_updownerrs("{$adminfile}/update/{$addr}/filelist.txt").dlerror($return),$type,$olid,$ver,$addr);
	}
	...
	foreach($strs as $addrskey=>$strsval){	
		...
		if($str){
			if(!filetest("../update/$addr/dateback/$strsvalto")){
				dl_error("{$adminfile}/update/{$addr}/dateback/{$strsvalto}{$lang_updaterr1}",$type,$olid,$ver,$addr);
			}
...
require_once '../system/database/global.func.php';
	...
	do{
		...
		if(trim($sqldump)){
			...
			if(!file_put_contents($bakfile, $sqldump)){
				dl_error($lang_updaterr2."({$adminfile}/databack/{$con_db_name}_{$date}_{$random}_{$num}.sql)",$type,$olid,$ver,$addr);
			}
		}
		$num++;
	}
	...
	if(!file_put_contents("../update/$addr/sqlist.php",$string)){
		dl_error($lang_updaterr2."({$adminfile}/update/{$addr}/sqlist.php)",$type,$olid,$ver,$addr);
	}
	$adminfile=$url_array[count($url_array)-2];
	...
	if($zip_list==0){
		dl_error($lang_updaterr3."({$adminfile}/databack/web/metinfo_web_".date('YmdHis',time()).".zip)",$type,$olid,$ver,$addr);
	}
	echo "{$lang_updaterr5}<script type=\"text/javascript\">setTimeout(function (){olupdate('$olid','$ver','dirpower');},500);</script>";
}
	...
	if($type==2){
		...
		$return=dlfile("$addr/filelist.txt","../update/$addr/filelist.txt");
		if($return!=1){		
			dl_error($lang_updownerrs.dlerror($return)."({$adminfile}/update/$addr/filelist.txt)",$type,$olid,$ver,$addr);
		}
	}
	$str=file_get_contents("../update/$addr/filelist.txt");
	$strs=explode('|',$str);
	...
	foreach($strs as $addrskey=>$strsval){
		...
		if(!filetest("../update/$addr/$strsval")){
			dl_error("{$adminfile}/update/{$addr}/{$strsval}{$lang_updaterr6}",$type,$olid,$ver,$addr);
		}
	}
	...
	if($return!=1){		
		dl_error($lang_updownerrs.dlerror($return)."({$adminfile}/update/$addr/check.php)",$type,$olid,$ver,$addr);
	}
	...
	if($return!=1){
		dl_error("{$lang_updaterr8}".dlerror($return)."({$adminfile}/update/$addr/dlfilelist.txt)",$type,$olid,$ver,$addr);
	}
	$return=dlfile("$addr/dladd.php","../update/$addr/dladd.php");
	if($return!=1){
		dl_error("{$lang_updaterr8}".dlerror($return)."({$adminfile}/update/$addr/dladd.php)",$type,$olid,$ver,$addr);
	}
		...
		if(!is_array($sqlfile)){
			...
			do{
				...
				if(trim($sqldump)){
					...
					if(!file_put_contents($bakfile, $sqldump)){
						dl_error($lang_updaterr2."({$adminfile}/update/$addr/{$con_db_name}_{$date}_{$random}_{$num}.sql)",$type,$olid,$ver,$addr);
					}
				}
				$num++;
			}
